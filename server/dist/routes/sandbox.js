...
function checkChinesePunctuation(code) {
    const found = [];
...
export async function sandboxRoutes(app) {
    function canRunCode(userId) {
        const user = Array.from(store.users.values()).find(u => u.id === userId);
        if (!user)
            return { allowed: false, reason: '用户未找到' };
...
function enhanceError(rawErr, code) {
    // 如果包含中文标点，优先处理
    ...
    if (msg.includes('SyntaxError') || msg.includes('syntax error')) {
        ...
        msg += '\n可能的原因：\n';
        if (rawErr.includes('invalid syntax'))
            msg += '  • 拼写错误或符号用错了\n';
        if (rawErr.includes('unexpected EOF'))
            msg += '  • 括号没有闭合，少了 )\n  • 引号没有闭合\n';
        if (rawErr.includes('EOL'))
            msg += '  • 字符串的引号没有闭合\n';
        if (rawErr.includes('unexpected indent'))
            msg += '  • 缩进不一致（混用了空格和 Tab）\n';
        msg += '\n💡 解决方法：\n';
        msg += '  1. 检查提示行附近的代码\n';
        msg += '  2. 确认所有括号 ( ) 都成对出现\n';
        msg += '  3. 确认字符串引号 " 或 \' 已闭合\n';
