...
export async function subscriptionRoutes(app: FastifyInstance) {
  ...
  app.post('/create', async (request, reply) => {
    const { planId, provider } = request.body as any
    ...
    try {
      ...
      if (planId === 'pro_monthly') endsAt.setMonth(endsAt.getMonth() + 1)
      else if (planId === 'pro_yearly') endsAt.setFullYear(endsAt.getFullYear() + 1)
      else if (planId === 'lifetime') endsAt.setFullYear(endsAt.getFullYear() + 100)
